...
function getI18n(_ref) {
  ...
  return (function () {
    function I18n(roots) {
      ...
      this.langFiles = _globby2.default.sync(_path2.default.join(roots.root, translations));
        ...
        return {
          after: function after(ctx) {
            ...
            if (!templatesGlob) return false;else {
              var globs = Array.isArray(templatesGlob) ? templatesGlob : [templatesGlob];

              _globby2.default.sync(globs.map(function (glob) {
                return _path2.default.join(ctx.roots.config.output, glob);
